...
「畫 ASCII 圖：3 台 server，1 台 LB。標示 Round Robin 和 Least Connections 分別在什麼場景比較好。」
...
**Micro-exercise (Build):**
「讀寫比 95:5 vs 50:50，畫出哪種 cache 策略最適合？為什麼？」
...
**Micro-exercise (Build):**
「用戶資料（profile）和用戶行為 log 用同一種 DB 好嗎？畫出你的推薦架構。」
...
**Micro-exercise (Build):**
「畫出黑五尖峰流量場景：沒有 MQ vs 有 MQ 時，訂單服務和庫存服務的行為差異。」
...
**Micro-exercise (Build):**
「設計一個 social media 的 API：手機版 feed 和網頁版 dashboard 分別需要什麼 endpoint？用 REST 還是 GraphQL？」
...
**Micro-exercise (Build):**
「畫出 JWT + refresh token 的流程：登入、存取 API、token 過期、刷新。」
...
**Micro-exercise (Build):**
「畫出 hash ring：4 個實體節點各有 3 個虛擬節點。標示一個 key 的定位過程和一台機器故障時的資料遷移。」
...
**Micro-exercise (Build):**
「電商場景：商品庫存和用戶評論，partition 時分別選 CP 還是 AP？為什麼？」
...
**Micro-exercise (Build):**
「社群媒體：發文 → 看自己的 timeline → 看朋友的 timeline。每一步需要什麼一致性等級？」
...
**Micro-exercise (Build):**
「3 個 replica 的 leader-based 設定：leader 掛了，畫出 failover 流程和可能的資料丟失窗口。」
...
**Micro-exercise (Build):**
「設計 Token Bucket：bucket size=100, refill rate=10/s。畫出 150 req 突發和 5 req/s 穩定流量的行為差異。」
...
**Micro-exercise (Build):**
「用戶回報延遲高。描述你用 logs、metrics、traces 各自能看到什麼、看不到什麼。」
...
**Micro-exercise (Build):**
「10 億筆資料，可接受 1% false positive。計算 Bloom Filter 需要多少 bit 和幾個 hash function。」
