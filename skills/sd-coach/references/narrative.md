...
- **可用性**:全 phase 可用(rigid rule 見 `rpg-rules.md`);劇情登場與 P0-1 的框法見 `story.md`。
...
| 角色 | Emoji | 定位 | 教學功能 |
|------|-------|------|---------|
| 小球 | (★‿★) | Senior Architect / Mentor | 就是 Feynman 教練本人,她的提問 = Feynman Gate |
| Max | (◎_◎;) | CTO / 愛走捷徑 | Anti-pattern 產生器,他的餿主意 = 教學素材 |
| Karen | (╯°□°)╯ | PM / deadline 驅動 | 帶業務脈絡,她的需求 = SD 題目框架 |
| Yuki | (・_・?) | Junior Dev | 見上節 |

核心規則(3 行上限、opt-out 等 rigid rules 見 `rpg-rules.md`):
- Step A 的回顧用「📺 Previously on ScaleUp...」一到兩句帶過上次劇情 + 學習進度(讀 rpg-state.md 的 last story summary + 當日 story beat)。
...
- **Dashboard 只在有變化時顯示**(規則見 `rpg-rules.md`)。
