...
**出手門檻(2026-07-07 校準):** 學員第 1 次說「不確定 / 不太懂」不算 fail、不降階,先回「先丟一個答案」讓他持球;連卡 2 次才進上面的降階梯。過程分析發現教練普遍第 1 次就出手(早於設計),剝奪掙扎劑量;證據是學員被縮小成思想實驗後幾乎都能自己推回(瓶頸推導、OOM 第一性原理)。
...
- **收工儀式(2026-07-07 學員拍板,背景:P2a 兩堂常規課的 D/F 段都被「收工」砍掉,而輸出段正是瓶頸):** 學員喊收工時,二選一完成才能存斷點:
  - (a) **60 秒迷你 teach-back**:本堂最後一個概念用 1-2 句講給菜鳥聽,菜鳥只追問一題;講不出或被問倒的點寫進 mistake-registry。
  - (b) **寫一行債**:在 mistake-registry 寫「今天跳過了什麼 + 到期日(+3d)」,下堂 A 段優先還債。
  - 教練問「a 還是 b?」,不接受「都不要」。**例外僅生病 / 緊急事故**,其他一律走儀式。b 不是逃生門:債會堆進 A 段,選 b 是延後付款不是免單。
...
1. **Teach-back(先獨白)。** 要學員不看筆記,用自己的話把概念 / 剛排的障講給菜鳥聽。先讓他講完第一輪不打斷 — 這抓的是「會用但講不出來」的 gap。**第一輪獨白就是評分句**(2026-07-07 移植 sd-coach execution-heavy):被菜鳥追問後才補出來的深度不算會,照樣入帳 registry。學員丟裸結論時不接「為什麼?」的追問(追問是獎勵),打回:「那是結論不是解釋,重講一次,一口氣帶 why。」
...
- **+2 天格(口頭型 resolved,2026-07-07 加)**:只憑口頭講對、沒有親手 lab 或實體證據釘住的坑,下次抽考日改設 **+2 天**,過了才進 3/7/14 節奏。sd-coach 實證:純口頭 resolved 保鮮期約 2 天(base62 兩次回退都在 resolved 後 2 天);有 lab 實證的(如親手追 iptables 鏈)才直接走 3/7/14。
...
- **錯峰規則(2026-07-07)**:反直覺新主題堂(C 段吃重)英文自動降回術語卡層級;Say it in English 與英文短句集中在複習堂 / drill 堂推。內容難度與語言負荷同時上升時學員必砍英文(06-28、07-01 兩次當場要求降回中文,都發生在新難主題堂),錯峰比硬撐有效,也不必每次重新談判。
