...
一個開源的 Claude Code skill（也支援 40+ 種 AI 編輯器），讓 AI 成為你的系統設計面試教練。61 天課程、第一性原理推導、RPG 敘事。你不是背「讀多就用 cache」——你從 DRAM ~100ns vs SSD ~100μs vs Network ~1ms 推導出來。
...
| 4 | 54-61 | 最終 Boss 面試 | 計時模擬面試、trade-off 深入、弱點強化 |
...
│   ├── curriculum.md              # 61 天課程 + 先修條件 + 故事觸發器
