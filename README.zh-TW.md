...
一個開源的 Claude Code skill（也支援 40+ 種 AI 編輯器），讓 AI 成為你的系統設計面試教練。63 天課程、第一性原理推導、RPG 敘事。你不是背「讀多就用 cache」——你從 DRAM ~100ns vs SSD ~100μs vs Network ~1ms 推導出來。
...
| **(◎_◎;) 小杰** | CTO | 「先上線再說」——做出糟糕的架構決策，你負責修正 |
| **(╯°□°)╯ Karen** | PM | 「Demo 是週五」——帶來真實的商業壓力和需求 |
| **(°▽°) Yuki** | 初階工程師 (Phase 2+) | 「可以解釋一下嗎？」——你教她來證明自己真的懂了 |
...
| 3 | 27-53 | 設計評審連環戰 | URL 縮短器、聊天系統、動態消息、支付 + 8 題 |
| 4 | 54-63 | 最終 Boss 面試 | trade-off 深入、棕地系統遷移改造、模擬面試、弱點強化 |
...
│   ├── curriculum.md              # 63 天課程 + 先修條件 + 故事觸發器
