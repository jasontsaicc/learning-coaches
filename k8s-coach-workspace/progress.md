...
| 目前主題 | **P2a chunk 1 進行中**(Service/kube-proxy/CoreDNS)。謎題A(Service→Endpoints→reconcile)✅;謎題B(ClusterIP虛擬→kube-proxy iptables DNAT→去中心化)**Gate 仍未過 🟡**(2026-06-27 用 Teach the Rookie 壓測:零件逐一逼問都認得,但 full teach-back 自己串整條時崩,且露出「封包先去 ClusterIP 拿 IP」反直覺誤解 → 進 mistake-registry,下次 A 段冷測);conntrack/CoreDNS/D段lab **未做** |
| 上次 session 日期 | 2026-06-27 |
| session 累計次數 (session_count) | 8 |
