...
| 目前主題 | **P2a chunk 1 全畢業 ✅(Service/kube-proxy/CoreDNS,含 D段 lab)**。謎題A/B/C + CoreDNS 全 ✅。**謎題B 已雙重封印**:2026-06-28 D段 lab 用 `docker exec <node> iptables-save` 親手追完整鏈(KUBE-SERVICES→KUBE-SVC 機率LB→KUBE-SEP DNAT--to-destination PodIP),worker/worker2 規則一致證去中心化 → 實體鐵證 + F段壓軸無鷹架全鏈 teach-back PASS(昨天崩的「封包先去ClusterIP拿IP」今天親口接對)。**下一步=chunk 2 Ingress**(或先 Weekly Review,已到期) |
| 上次 session 日期 | 2026-06-28 |
| session 累計次數 (session_count) | 9 |
...
> 下次繼續(一句話):**P2a chunk 1 D段 lab(叢集已起好,context=kind-k8s-coach-p0)**。腳本:apply backend Deployment(replicas≥2)+ ClusterIP Service → `get svc`(看 ClusterIP)`get endpoints`(看健康 Pod IP 清單)→ **重頭戲:進某台 node `docker exec <node> iptables-save | grep <clusterip>` 親眼看 DNAT 規則就刻在本機 kernel → 用實體證據冷確認「封包不拜訪 ClusterIP、改寫在發送端本機」打死昨天誤解 → 才標謎題B ✅** → `scale` backend 看 Endpoints 即時增減 → 起一個 client Pod `nslookup backend` 看 CoreDNS 把名字解析成 ClusterIP(驗證 CoreDNS)。做完 chunk 1 全綠,F 段迷你 mock 後進 chunk 2 Ingress。phase-2a 教材檔仍未建,從第一性原理教。**English Ramp:user 全域已改 70% EN/30% 中(2026-06-28 CLAUDE.md),但學員今天明說「中文佔比多一些」→ 中文為主、術語錨點英文,Say-it-in-English 輕推不硬逼**。EKS P2a 起可進場(terraform 指令由 user 親手跑,命名 `billing-dev-eks-*`)。**⚠️ Weekly Review 已到期(9-0≥7),chunk 1 收完(或下次 A 段)該插一次**。
