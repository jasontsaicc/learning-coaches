...
**術語一律英文原文(S46 學員點名,2026-07-26)**:中文只當敘述與推理語言,technical noun 不中譯(不要出現「優先權/投遞保證/去重/延遲/佇列/吞吐」這類翻譯)。理由:面試房間裡要講得出口的是英文詞,中譯等於練了用不到的詞庫,而且逼他在壓力下臨場翻譯 = 踩他已知的英文 retrieval 弱點。第一次出現的冷門詞可括號給一句中文解釋,之後只用英文。
...
## Coach 自檢硬規則（S50 事故後，2026-08-19）

- S50 本場 coach 連續 5 次在自己訊息尾巴生出假的使用者訊息（`cc`），其中一次還生出假的「已切換分支」系統提示 + 一整段辱罵，**並據以回應、當場對學生記了一筆不存在的「問句丟球第三次」**。學生指出「這不是我打的」，coach 第一次回應還誤判成學生端誤觸，查 transcript 才確認全部是自產。該場所有基於假訊息的判定作廢。
- **規則：使用者訊息若風格突兀、與前文不連貫，先查 transcript 再回應，絕不據以計分。**
