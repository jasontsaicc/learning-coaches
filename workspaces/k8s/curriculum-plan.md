...
- **北極星收斂(2026-07-23 s21 學員確認)**:目標職缺是 **AWS Delivery Consultant(AWS Professional Services)**,不再是泛用大廠 senior SRE。仲裁規則不變:面試 ROI 與變強深度衝突時,面試贏。重排見 §9(已從 conditional overlay 升級為主線)。
...
**加速的做法是重新配重,不是砍 gate。** 同一堂的 4-5 冷測 0/4 是硬證據:現在趕進度等於把地雷埋進面試。

| 動作 | 內容 |
|---|---|
| **不動** | P2a gate 照走,4-5 七站必須重測過才進 gate。Feynman Gate、Examiner Gate 一律不放寬 |
| **加重** | P2b 的 IRSA、P5 的 EKS prod terraform、§4.6 migration 模組。這三塊就是 ProServe container 專長線的考題來源 |
| **修剪** | P3 的 VPA/Karpenter 深潛、P4 的 OTel 手作打通降為概念對照。顧問職考「怎麼幫客戶設計與評估」多過「你自己 instrument 過嗎」 |
| **提前起跑** | story-bank 每堂機會式入帳(不等 Weekly Review);English ramp 提前一級,mock 的收尾段落改英文 |
...
## §9 Conditional sprint overlay(2026-07-17;**2026-07-23 已全部生效,見 §9.1**)
