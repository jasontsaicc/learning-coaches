...
| conntrack | /ˈkɒn.træk/ | The Linux kernel netfilter table that remembers each NAT'd connection so reply packets can be rewritten back. When it fills up, NEW connections get dropped while existing ones keep working. | NAT 的「記憶體」:去程 DNAT 改了 A→B 就記一筆,回程才反向改回去;table full=新連線被 drop、舊連線不受影響;屬 node kernel 層,kubectl 看不到(查法 conntrack -L / nf_conntrack_count vs _max / dmesg) | 2026-06-28 | 2026-07-24 (2026-07-14 重抽沒過,教練四輪後**直給**兩個詞。**2026-07-17 兩個詞 PASS,推 +7**:第一輪仍答「DNAT / connect」=直給的東西 3 天後完全蒸發;教練改給**框架不給答案**〔「封包信封上只有兩個欄位:來源、目的地。答案空間只有 2 個選項」〕+ shop 真數字推導〔目的地 10.96.100.231 是假 IP、沒人擁有=非改不可;來源是真的、能用=不用改〕→ **一次自產「去程改目的地、回程改來源」**。精準度補刀:是「不用改」不是「不能改」,SNAT 場景真的會改來源,P3 再碰。**分工句仍未收**〔kube-proxy 只寫規則、kernel 搬封包;去程查規則、回程查帳本〕,s17 補收,可寄生 NetworkPolicy「回程免開洞」段。**此卡是「給框架 vs 給答案」的對照組證據,見 progress.md s16**) |
