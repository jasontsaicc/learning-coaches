...
- **s38(2026-09-21)**:同堂三次裸結論(etcd 輸出預測「B」、KMS 開啟後開發者看不看得到「B」、volume 那格「cat 是最新的」),都要追問才補。正樣本:「甲,因為他沒有 KMS 解密的權限」、「env 在啟動那一刻寫進 memory,之後副本不會同步」、ECS task 啟動抓 SSM 的自發類比。
...
- **s38(2026-09-21)續犯,換皮成 KMS 版**:已開 KMS,CI SA(有 `get secrets`)的 token 被偷,問調哪一層縮損害 → **首答「KMS 的層級」**;前一題「誰把密文變明文」答「kubectl 自己解密」。銀行比喻(金庫=etcd、櫃員=API Server、身分證=RBAC)+ 二選一後答 B「縮小爆炸範圍」(supported)。根因同一條:把「資料怎麼存」的軸當成「誰能拿」的軸。09-24 抽:同一 CI SA 情境不給選項,要求自己說出層名 + 怎麼調(resourceNames 收窄 / 拿掉 get secrets / 改 ESO+IRSA)。
