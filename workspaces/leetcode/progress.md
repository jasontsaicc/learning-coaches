...
1. **開場先收 #543 的溫度計 ②**(已出題,學員未答):記帳那行的 `max` 拿掉、寫成
   ...
   等於退化成「只算穿過 root 的路」。答不出來就指帳本分鏡第 04→06 格。
2. 開場默寫改考**雙軌模板**(`self.ans` 一行 + 軌道 A/B 兩行)。這是第 1 次考。
   `dfs` 三格模板前三次都沒過,但 9/11 #543 冷寫一次過,可視為已收斂,不用再單考
3. 補 #104 / #543 的 `solution.py`(目前兩題都只有 drill.py)
4. 之後 #100 Same Tree(換皮:同骨架,回傳值換成 bool)、
   #124 Max Path Sum(同雙軌模板,只換那兩行 —— notes.md 有對照表)
