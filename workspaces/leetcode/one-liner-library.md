...
| Reverse Linked List / #206 (Linked List) | 單條就地反轉 → 每個 node 的 `next` 從指下一棒改成指前一棒;三指標 `prev`(新鏈的頭,起手 **`None`**)、`curr`(走路指標,起手 `head`)、`nxt`(暫存)。`while curr` 每圈 **存、改、挪、挪**:`nxt = curr.next`(先存,下一行要蓋掉它)→ `curr.next = prev`(掉頭)→ `prev = curr` → `curr = nxt`;跳出時 `curr` 撞到 `None`,回傳 **`prev` 不是 `curr`**。三個死穴: ① `prev` 起手是 `None` 不是 `head`(原 head 反轉後是尾巴、`.next` 要 `None`;寫 `head` 會讓前兩個 node 互指成環) ② 順序不能動,先「挪」再「改」→ 留在後面的 node 永遠不會掉頭 ③ 暫存變數叫 `nxt` 別叫 `next`(shadow builtin,手滑打成 `next` 會 `AttributeError`)。O(n) 時間 O(1) 空間;遞迴版爆 stack。換皮鏈: 21 / 143 / 19 / 25 都是這骨架加東西 |
| Merge Two Sorted Lists (Linked List) | 兩條**已排序**合併 → 最小值一定是兩個 head 之一 (invariant)，所以兩個 pointer 各站一列、每圈挑小的接走即可，完全不用排序；**dummy 假車頭**用一節常數空間換掉「接第一節」的 special case，`tail` 永遠站隊尾；跳出 while 時至少一邊是 None，`tail.next = list1 or list2` 把剩下的**整段**一次掛上 (剩下的都已排序且 ≥ 已接的)；回傳 `dummy.next` 不是 `dummy`。三個死穴: ① `return dummy.next` 縮排必須在 while **外面** ② `tail = tail.next` 必須在 if/else **外面** (每圈都要跟上) ③ while 條件是 `and` 不是 `or` (一邊空了再讀 `.val` 會 AttributeError)。`<=` 而非 `<` 保住 stable，merge sort 靠這性質。推廣: k 條只需把「兩兩比」換成 min-heap → #23，骨架 8 行不動 — O(n+m) 時間 O(1) 空間 |
| Heap k-way merge / #23 (Linked List, Heap) | k 條已排序 lists 合併 → invariant: heap 裡永遠恰好放著**每條 list 目前的 head**，root 一定是全域剩餘最小值；push 全部 head 進 heap，每次 pop 最小的接到 tail、立刻補它同一條 list 的 next 進 heap，heap 空即完成 (骨架就是 #21 的 dummy+tail，只是選最小的方式換成 heap)。tuple 要放 `(val, list_id, node)` 不是 `(val, node)` — val 打平時 Python 接著比第二個元素，只放 node 會比到 `ListNode`（沒 `__lt__`）直接 `TypeError`；同一時刻每個 list_id 在 heap 裡最多一筆，tie 永遠靠 list_id 分勝負，node 那格永遠不會真的被比。換掉 merge-one-by-one 每輪重新掃全部的 O(kN)，變成每個 node 進出各 O(log k) — O(N log k) 時間 O(k) 空間。follow-up: 兩兩配對用 #21 做 divide and conquer，log k 層每層 O(N)，同樣 O(N log k) |
